extends Resource
class_name QuantumRuntimeThemeConfig
...
@export_file("*.png", "*.webp") var hud_score_frame_path := "res://assets/themes/candy_sky_islands/hud_score_frame_clean_9router.png"
@export_file("*.png", "*.webp") var candy_skybox_path := "res://assets/themes/candy_sky_islands/sky_panel_islands.png"
...
func validate_runtime() -> Array[String]:
	...
	for path in [
		...
		if path.strip_edges().is_empty():
			errors.append("runtime asset path is empty")
		elif not ResourceLoader.exists(path) and not FileAccess.file_exists(path):
			errors.append("missing runtime asset path: %s" % path)
	return errors
