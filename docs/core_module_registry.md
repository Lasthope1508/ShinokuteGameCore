...
| SceneTransitionLifecycle | ACTIVE | `ux.transition.lifecycle` | `all-casual`, `mobile-web` | `addons/shinokute_game_core/ux/scene_transition_lifecycle.gd` | `Tests/test_core_ux_contract.gd` | Need scene transition busy guard and fade/change/fade lifecycle reports. | Do not put transition shader, animation node, route paths, or actual scene loading policy in lifecycle. |
...
| PublishAudit | ACTIVE | `runtime.publish.audit` | `debug-publish`, `mobile-web` | `addons/shinokute_game_core/runtime/publish_audit.gd` | `Tests/test_runtime_core_p0_p1_contract.gd` | Need pure-data manifest validation, forbidden marker checks, export preset text checks, and hosting header checks. | Do not claim publish-ready from audit helpers alone or perform filesystem/export actions inside this module. |
...
| ProjectileHitBudget | ACTIVE | `combat.projectile.hit-budget` | `shooter-survivor`, `tower-defense`, `roguelike`, `rpg` | `addons/shinokute_game_core/runtime/projectile_hit_budget.gd` | `Tests/test_runtime_core_p0_p1_contract.gd` | Need generic projectile hit counts, hit id history, rehit cooldown, and expire reasons. | Do not decide damage, collision, faction, projectile art, or pierce meaning in core. |
| ProjectileTravelRuntime2D | ACTIVE | `combat.projectile.travel` | `shooter-survivor`, `tower-defense`, `arcade`, `rpg` | `addons/shinokute_game_core/runtime/projectile_travel_runtime_2d.gd` | `Tests/test_runtime_core_p0_p1_contract.gd` | Need generic projectile position stepping, distance/lifetime/range expiry, and optional angular steering toward caller-owned target positions. | Do not decide collision, damage, target validity, projectile ids, pool return, art, VFX, or homing target selection. |
...
| AttackCadence | ACTIVE | `combat.attack.cadence` | `shooter-survivor`, `tower-defense`, `arcade`, `rpg` | `addons/shinokute_game_core/runtime/attack_cadence.gd` | `Tests/test_runtime_core_p0_p1_contract.gd` | Need cooldown, anticipate, duration, recovery, and ready-phase reports for attack timing. | Do not spawn projectiles, read input, choose targets, apply damage, or trigger animation/audio. |
