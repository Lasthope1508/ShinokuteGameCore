...
For a new game or test game, build assets as reusable blocks before final art:

1. Define the block kit in the game-local `docs/asset_manifest.md`.
2. Make the smallest useful visual pieces first:
   - logo placeholder,
   - button shell,
   - panel shell,
   - input shell,
   ...
   - VFX placeholder.
3. For each block, record role, source, path, owner rect, padding, and
   In-game Size before wiring scenes.
4. Build a small asset test scene that places every block in its intended
   control role.
5. Capture desktop and mobile screenshots of the test scene.
6. Only after the block kit fits, wire assets into real gameplay screens.
