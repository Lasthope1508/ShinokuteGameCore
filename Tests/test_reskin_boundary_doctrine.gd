...
func _init() -> void:
	...
	_assert_true(doctrine.contains("Core wiring is logic/service wiring only; it is not production feature completion."), "doctrine should say core wiring is not feature completion")
	_assert_true(doctrine.contains("Every enabled shared core feature must have game-owned function-skin UI before the feature can be marked done."), "doctrine should require game-owned UI for enabled core features")
	_assert_true(doctrine.contains("username, leaderboard, result, settings, menus, ads, profile, and publish prompts"), "doctrine should name common shared features that need game UI")
	_assert_true(doctrine.contains("A shared feature is not done until game-owned UI exists, is theme/SSOT backed, and has contract tests plus screenshot validation."), "doctrine should require SSOT, tests, and screenshots for shared feature UI")
	_assert_true(doctrine.contains("Definition Resolver Boundary"), "doctrine should document generic definition resolver boundary")
	_assert_true(doctrine.contains("Core owns id-based definition resolution, duplicate-id validation, missing-reference reporting, and weighted id selection only."), "doctrine should keep definition resolver generic")
	_assert_true(doctrine.contains("Games own definition meaning, domain keys, stat application, enemy behavior, projectile behavior, inventory semantics, and upgrade effects."), "doctrine should keep domain semantics game-owned")
	_assert_true(doctrine.contains("Function skin owns picker panels, option cards, labels, icons, animation, and text layout for any resolved options."), "doctrine should keep resolved option presentation UI-owned")
	...
	_assert_true(checklist.contains("Core feature wiring is logic/service only; feature completion also requires game-owned UI/function skin."), "checklist should separate core wiring from UI completion")
	_assert_true(checklist.contains("Each enabled shared feature has game-owned UI"), "checklist should require per-feature UI")
	_assert_true(checklist.contains("Shared feature UI uses game SSOT/theme assets"), "checklist should require SSOT-backed shared feature UI")
	_assert_true(checklist.contains("Contract test and screenshot validation exist for each enabled shared-feature UI"), "checklist should require test and screenshot evidence")
	...
	_assert_true(agents.contains("Do not mark profile, username, leaderboard, settings, ads, menus, result, or publish features complete after code wiring only."), "agents guide should forbid logic-only completion")
	...
	_assert_true(readme.contains("Core wiring does not complete production UI."), "root README should state core wiring does not complete UI")
	_assert_true(addon_readme.contains("reskin_core_skin_boundary.md"), "addon README should link doctrine")
	_assert_true(addon_readme.contains("Every production game must provide its own UI/function skin for enabled shared features."), "addon README should state games own shared feature UI")
