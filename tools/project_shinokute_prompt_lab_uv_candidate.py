...
def _add_atlas_material(mesh_obj: bpy.types.Object, atlas_path: Path) -> None:
    ...
    material = bpy.data.materials.new("Shinokute_Project_From_PromptLab_Albedo")
...
def _project_uv(mesh_obj: bpy.types.Object, meta: dict) -> None:
    ...
    uv_layer = mesh_obj.data.uv_layers.get("ShinokutePromptLabProjection")
    if uv_layer is None:
        uv_layer = mesh_obj.data.uv_layers.new(name="ShinokutePromptLabProjection")
...
def main() -> None:
    ...
    mesh = mesh_objs[0]
    mesh.name = "Shinokute_Hunyuan_PromptLab_Projected"
    _project_uv(mesh, meta)
    _add_atlas_material(mesh, atlas_path)
    mesh["shinokute_projection_source"] = "prompt lab albedo attempt2; Photoroom full sheet first; high-alpha component polygon extraction"
