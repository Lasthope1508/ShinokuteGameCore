	...
	for y in range(height):
		for x in range(width):
			...
			if idx < grid_array.size():
				...
				# Create a deep copy of ports, enforcing source/target safety
				var ports = []
				var is_src = (Vector2i(x, y) == source_pos)
				var is_tgt = (Vector2i(x, y) == target_pos)
				...
				var tile = {
					...
					"ports": ports,
					"rotation": 0 if (is_src or is_tgt) else int(cell_data.get("rotation", 0))
				}
...
func rotate_tile(x: int, y: int) -> void:
	...
	var pos := Vector2i(x, y)
	if pos == source_pos:
		source_ports = ports.duplicate()
	elif pos == target_pos:
		target_ports = ports.duplicate()
