...
func test_level_generator() -> bool:
	...
	if not assert_true(lvl1 != {}, "Generator should produce a non-empty level dict"):
		return false
	if not assert_true(lvl1.get("width") == 5 and lvl1.get("height") == 5, "Level 1 size should be 5x5"):
		return false
	if not assert_true(lvl1.get("grid").size() == 25, "Level 1 grid size should be 25"):
		return false
	if not assert_true(lvl1.get("source").get("x") == 0 and lvl1.get("source").get("y") == 0, "Level 1 source should stay top-left"):
		return false
	if not assert_true(lvl1.get("target").get("x") == 4 and lvl1.get("target").get("y") == 4, "Level 1 target should stay bottom-right"):
		return false
	...
	if not assert_true(lvl6.get("width") == 6 and lvl6.get("height") == 6, "Level 6 size should scale up to 6x6"):
		return false
