...
func _init() -> void:
	...
	for required_text in [
		...
		"Store settings/leaderboard modal portrait/landscape size ratios in ThemeConfig and place modal rect from viewport ratios.",
		...
		"Solved/win popup uses generated `modal_frame`, non-container root, and role-specific compact result ratios/margins/action width/action height from `ThemeConfig.ui_result_modal_*`; it must never reuse generic `ui_modal_*` dimensions or full-width modal buttons.",
