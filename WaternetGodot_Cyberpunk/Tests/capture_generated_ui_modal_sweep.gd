...
const CAPTURES = [
	{
		...
		"path": "res://debug/generated_ui_settings_dark_portrait.png"
	},
	{
		"mode": "dark",
		"size": Vector2i(1280, 720),
		"path": "res://debug/generated_ui_settings_dark_landscape.png"
	},
	{
		"mode": "light",
		"size": Vector2i(720, 1280),
		"path": "res://debug/generated_ui_settings_light_portrait.png"
	},
	{
		"mode": "light",
		"size": Vector2i(1280, 720),
		"path": "res://debug/generated_ui_settings_light_landscape.png"
	}
]
	...
	if capture_index >= CAPTURES.size():
		...
		if passed:
			for capture in CAPTURES:
				print("capture_generated_ui_modal_sweep: %s" % String(capture["path"]))
			print("capture_generated_ui_modal_sweep: PASS")
			quit(0)
		else:
			print("capture_generated_ui_modal_sweep: FAIL")
			quit(1)
		return
...
func _prepare_capture(index: int) -> void:
	...
	if scene != null:
		...
		var settings_overlay = scene.get_node_or_null("HUD/SettingsOverlay")
		if settings_overlay != null:
			settings_overlay.visible = true
		if scene.has_method("_update_settings_buttons"):
			scene._update_settings_buttons()
		if scene.has_method("_recalculate_layout"):
			scene._recalculate_layout()
		scene.queue_redraw()
	frame_count = 0
...
func _capture(path: String, mode: String) -> void:
	if scene == null:
		...
		return
	var settings_overlay = scene.get_node_or_null("HUD/SettingsOverlay")
	_assert_true(settings_overlay != null, "Settings overlay should exist")
	_assert_true(settings_overlay != null and settings_overlay.visible, "Settings overlay should be visible")
	_assert_true(settings_overlay != null and settings_overlay.get_node_or_null("GeneratedModalFrame") is TextureRect, "Settings overlay should use generated modal frame")
	var image := root.get_texture().get_image()
	if image == null:
		_fail("Viewport image is null")
		return
	_assert_true(_image_has_variation(image), "Generated modal capture should not be blank")
