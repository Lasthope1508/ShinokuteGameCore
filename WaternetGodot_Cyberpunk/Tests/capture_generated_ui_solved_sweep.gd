...
const CAPTURES = [
	{
		...
		"path": "res://debug/generated_ui_solved_dark_portrait.png"
	},
	{
		"mode": "dark",
		"size": Vector2i(1280, 720),
		"path": "res://debug/generated_ui_solved_dark_landscape.png"
	},
	{
		"mode": "light",
		"size": Vector2i(720, 1280),
		"path": "res://debug/generated_ui_solved_light_portrait.png"
	},
	{
		"mode": "light",
		"size": Vector2i(1280, 720),
		"path": "res://debug/generated_ui_solved_light_landscape.png"
	}
]
	...
	if capture_index >= CAPTURES.size():
		...
		if passed:
			for capture in CAPTURES:
				print("capture_generated_ui_solved_sweep: %s" % String(capture["path"]))
			print("capture_generated_ui_solved_sweep: PASS")
			quit(0)
		else:
			print("capture_generated_ui_solved_sweep: FAIL")
			quit(1)
		return
...
func _prepare_capture(index: int) -> void:
	...
	if scene != null:
		...
		var solved_popup = scene.get_node_or_null("HUD/SolvedPopup")
		if solved_popup != null:
			solved_popup.visible = true
		var popup_moves = scene.get_node_or_null("HUD/SolvedPopup/MarginContainer/VBoxContainer/PopupMoves")
		if popup_moves is Label:
			(popup_moves as Label).text = "MOVES USED: 12"
		if scene.has_method("_recalculate_layout"):
			scene._recalculate_layout()
		scene.queue_redraw()
	frame_count = 0
...
func _capture(path: String, mode: String) -> void:
	if scene == null:
		...
		return
	var solved_popup = scene.get_node_or_null("HUD/SolvedPopup")
	_assert_true(solved_popup != null, "Solved popup should exist")
	_assert_true(solved_popup != null and solved_popup.visible, "Solved popup should be visible")
	_assert_true(solved_popup != null and solved_popup.get_node_or_null("GeneratedSolvedModalFrame") is TextureRect, "Solved popup should use generated modal frame")
	var image := root.get_texture().get_image()
	if image == null:
		_fail("Viewport image is null")
		return
	_assert_true(_image_has_variation(image), "Generated solved modal capture should not be blank")
